...
/**
 * This class contains the board data represented as a two dimensional array of
...
final class BoardModel implements TableModel {
	...
	/**
	 * Set a cell to contain a given mark and return true of this gives 5 in a
	 * row.
	 * 
	 * @param x
	 *            X coordinate of cell to update
	 * @param y
	 *            Y coordinate of cell to update
	 * @param mark
	 *            Mark to assign to (x,y)
	...
	boolean setCell(int x, int y, char mark) {
		...
		return checkHorizontal(x, y) || checkVertical(x, y)
				|| checkDiagonalUp(x, y) || checkDiagonalDown(x, y);
	}
	...
	private boolean checkHorizontal(int x, int y) {
		// Find the minimum and maximum x values for this y which have the same
		// mark
		int minX = x;
		while (minX > 0 && boardCells[minX - 1][y].equals(boardCells[x][y]))
			minX--;
		int maxX = x;
		while ((maxX + 1) < boardCells.length
				&& boardCells[maxX + 1][y].equals(boardCells[x][y]))
			maxX++;
	...
	private boolean checkVertical(int x, int y) {
		// Find the minimum and maximum y values for this x which have the same
		// mark
		int minY = y;
		while (minY > 0 && boardCells[x][minY - 1].equals(boardCells[x][y]))
			minY--;
		int maxY = y;
		while ((maxY + 1) < boardCells.length
				&& boardCells[x][maxY + 1].equals(boardCells[x][y]))
			maxY++;
		// If the difference is larger than 4, we have 5 in a row
		return maxY - minY >= 4;
	...
	/**
	 * Check if (x,y) is part of a diagonal 5 in a row. Only checks diagonals
	 * going from bottom left to upper right.
	 * 
	 * @param x
	 *            X coordinate of cell to check
	 * @param y
	...
	private boolean checkDiagonalUp(int x, int y) {
		...
		int minY = y;
		while (minX > 0 && minY > 0
				&& boardCells[minX - 1][minY - 1].equals(boardCells[x][y])) {
			minX--;
			minY--;
		}
		int maxX = x;
	...
	/**
	 * Check if (x,y) is part of a diagonal 5 in a row. Only checks diagonals
	 * going from upper left to bottom right.
	 * 
	 * @param x
	 *            X coordinate of cell to check
	 * @param y
	...
	private final class Cell {
		...
		public boolean equals(Object obj) {
			return obj instanceof Cell && contents == ((Cell) obj).contents;
		}
	}
}
