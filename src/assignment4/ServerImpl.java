 ...
 * <p/>
 * The transactions to be performed can be chosen randomly or they can be
 * specified by an input file. The operations of the system are logged to an
 * output file. It's important to keep this log detailed, as it will be used as
 * "proof" of the system's performance (whether or not it is able to detect and
 * resolve deadlocks).
 * <p/>
 * The input file is specified as an optional second command line parameter (the
 * first parameter specifies the registry address). See example_input_file for
 * an example of what such an input file may look like. If no input file is
 ...
 * to the log. The name of the output (log) file is specified in the Globals
 * class, but may be changed by the input file.
 * <p/>
...
public class ServerImpl extends UnicastRemoteObject implements Server {
	...
	public ServerImpl(String ip, String inputfile) throws RemoteException {
		...
		try {
			// Find the registry proxy object used to bind non-local objects to
			// the registry:
			registryProxy = (RegistryProxy) Naming.lookup("rmi://" + ip
					+ "/RegistryProxy");
		} catch (NotBoundException nbe) {
			System.out
					.println("Couldn't find RegistryProxy object in registry. Be sure to use the StartRegistry class to start up your registry.");
			System.exit(1);
			...
			System.err
					.println("Couldn't connect to the RMI registry. Make sure that a RMI registry is running on the address you specified.");
			System.exit(1);
		}
		...
		try {
			...
			output = new PrintWriter(new FileWriter(Globals.OUTPUT_FILE_PREFIX
					+ "_server_" + serverId + ".txt", true));
			output.println("==================================================================");
		...
		// Start up the GUI
		gui = new Gui("Server " + serverId, resources, servers, this,
				(serverId - 1) % 4);
	...
	/**
	 * Read the global parameters specified in the beginning of the input file.
	 * 
	 * @param inputfile
	 *            The name of the inputfile, or null if no input file is used.
	 */
	private void readGlobalParameters(String inputfile) {
		if (inputfile != null) {
			...
			if (line == null || !line.startsWith("TEST CASE: ")) {
				System.out
						.println("ERROR: Input file is incorrectly formatted. Program aborted.");
				System.exit(1);
			}
			...
			Globals.NOF_RESOURCES_PER_SERVER = Integer.parseInt(line
					.substring(26));
			line = input.readLine();
	...
	/**
	 * Outputs a text string to the user, adding a newline at the end, and logs
	 * it.
	 * 
	 * @param s
	 *            The text to display.
	...
	/**
	 * Outputs a text string relevant to a specific transaction to the user, and
	 * logs it.
	 * 
	 * @param s
	 *            The information to display.
	 * @param tId
	 *            The ID of the transaction relevant to this information.
	 */
	...
	/**
	 * Outputs a text string relevant to a specific transaction to the user,
	 * formated with a newline at the end, and logs it.
	 * 
	 * @param s
	 *            The information to display.
	 * @param tId
	 ...
	 * 
	 * @param name
	 *            The name specifying the object to look up in the registry.
	 * @return The ID of the server that was looked up, or -1 if the server
	 *         could not be found.
	 */
	 ...
	 * 
	 * @param asId
	 *            The ID to attempt to claim. If other servers are trying to
	 *            bind to the registry at the same time, a larger ID may be
	 *            used.
	 */
	...
	/**
	 * Called to notify this server that a new server has connected to the
	 * system. Adds the new server to this server's hash table of known servers.
	 * 
	 * @param s
	 *            The server that arrived.
	 */
	public synchronized void newServerConnected(Server s)
			throws RemoteException {
		int id = s.getServerId();
		println("Server " + serverId
				+ " was notified of the arrival of server " + id);
		servers.put(id, s);
		if (gui != null)
	...
	/**
	 * Called to notify this server that a server has disconnected from the
	 * system. Removes the disconnected server from this server's hash table of
	 * known servers.
	 * 
	 * @param disconnectedId
	 *            The ID of the server that left.
	 * @param messengerId
	 *            The ID of the server that first discovered that that server
	 *            had left.
	 */
	public void serverDisconnected(int disconnectedId, int messengerId)
			throws RemoteException {
		println("Server " + serverId
				+ " was notified of the departure of server " + disconnectedId
				+ " by server " + messengerId + '.');
		serverDisconnected(disconnectedId);
	}

	/**
	 * Removes the specified server from this server's hash table of known
	 * servers, and unlocks any local resources locked by transactions from that
	 * server.
	 * 
	 * @param disconnectedId
	 *            The ID of the server that has disconnected.
	 */
	private synchronized void serverDisconnected(int disconnectedId) {
		...
		// Unlock any resources locked by transactions from the server that
		// disconnected
	...
	/**
	 * Gives the lock of the specified local resource to the specified
	 * transaction. This method blocks until the lock has been acquired.
	 * 
	 * @param lockerId
	 *            The ID of the transaction that wants the lock.
	 * @param resourceID
	 *            The ID of the resource whose lock the transaction wants.
	...
	/**
	 * Release the lock of the specified local resource, which is currently
	 * locked by the specified transaction.
	 * 
	 * @param lockerId
	 *            The ID of the transaction that owns the lock and wants to
	 *            release it.
	 * @param resourceID
	 *            The ID of the resource whose lock the transaction wants to
	 *            release.
	 * @return Whether or not the lock could be released.
	 */
	public boolean releaseLock(int transactionId, int resourceId)
			throws RemoteException {
	...
	/**
	 * Called whenever this server loses contact with another server. Notifies
	 * all servers that the given server is down.
	 * 
	 * @param id
	 *            The ID of the server with which we lost contact.
	 */
	void lostContactWithServer(int id) {
		println("Contact with server " + id + " was lost.");
		// Notify all other known servers that the server with the given id is
		// down.
	...
	/**
	 * Gets the ID of the server running the specified transaction. The server
	 * ID can be deduced by the transaction ID.
	 * 
	 * @param transactionId
	...
	/**
	 * Called by the GUI when the Start button was pressed. Ask the other known
	 * servers to start as well.
	 */
	void start() {
	...
	/**
	 * Loop executing transactions on this server. Random transactions are
	 * executed one at a time, until the total number of transactions executed
	...
	private void runTransactions() {
		while (transactionCounter < Globals.MAX_NOF_TRANSACTIONS_PER_SERVER) {
			if (!gui.getPauseStatus()) {
				int newId = serverId * Globals.MAX_NOF_TRANSACTIONS_PER_SERVER
						+ transactionCounter;
				...
				Globals.randomSleep(Globals.MIN_ARRIVAL_WAIT,
						Globals.MAX_ARRIVAL_WAIT);
			} else {
	...
	/**
	 * Loop executing transactions on this server. The transactions are
	 * described by the input file.
	 */
	private void runTransactions(TransactionFileReader input) {
		String line = input.readLine();
		while (line != null
				&& transactionCounter < Globals.MAX_NOF_TRANSACTIONS_PER_SERVER) {
			...
			if (!gui.getPauseStatus()) {
				...
				println("Transaction " + newId + " ended.");
				gui.updateTransactionCounters(nofAborts, transactionCounter);
				// Read a line describing the waiting time until the next
				// transaction arrives.
				// The line should be on the format WAIT 50-200 (signifying a
				// wait between 50 and 200 ms).
				line = input.readLine();
	...
	/**
	 * Picks a random resource (from the entire distributed system) that is not
	 * already in the given list of resources.
	 * 
	 * @param resourceAccesses
	 *            A list of resources not to pick.
	 * @return A random resource not already present in the resourceAccesses
	 *         list.
	 */
	synchronized ResourceAccess getRandomResource(
		...
		do {
			...
			int resourceId = Globals.random(0,
					Globals.NOF_RESOURCES_PER_SERVER - 1);
			result = new ResourceAccess(servers.get(keys[randomPos]),
					keys[randomPos], resourceId);
	 ...
	 *            address of the RMI registry, and the second specifies the name
	 *            of the input file. If no parameters are supplied, the default
	 *            address of "localhost:1111" is used, and transactions are
	 *            performed randomly.
	 */
