...
/**
 * A transaction in the distributed transaction system. A transaction consists
 * of a series of accesses to resources in the distributed system. Two-phase
 * locking is used. Which and how many resources to access is determined
...
class Transaction {
	...
	/**
	 * Signalling variable used to exchange messages between different threads
	 * operating on this transaction
	 */
	...
	/**
	 * The transaction file reader whose input specifies the contents of this
	 * transaction
	 */
	 ...
	 *            The server executing this transaction.
	 * @param input
	 *            The input file reader specifying the contents of this
	 *            transaction. If this parameter is null, the transaction is
	 *            performed randomly.
	 */
	...
	/**
	 * Executes this transaction. This method blocks the calling thread until
	 * the transaction has completed, which may take a while.
	 * 
	...
	boolean runTransaction() {
		...
		owner.println("Starting transaction " + transactionId + '.',
				transactionId);

		// Figure out how many resource accesses the transaction consists of.
		int nofAccesses = Globals.random(
				Globals.MIN_NOF_ACCESSES_PER_TRANSACTION,
				Globals.MAX_NOF_ACCESSES_PER_TRANSACTION);
		if (input != null) {
	...
	/**
	 * Processes a resource that the transaction has acquired the lock to. No
	 * actual processing is done, but some milliseconds are spent sleeping to
	 * simulate the processing.
	 */
	private void processResource() {
		// Figure out how long to "process" the resource:
		if (input == null)
			Globals.randomSleep(Globals.MIN_PROCESSING_TIME,
					Globals.MAX_PROCESSING_TIME);
		else {
			// Read the processing time from the input file.
			// The line should be on the format PROCESS 50-200 (signifying a
			// processing time between 50 and 200 ms).
			String line = input.readLine();
	...
	private ResourceAccess getNextResource() {
		if (input == null)
			...
			return new ResourceAccess(owner.getServer(serverId), serverId,
					resourceId);
		}
	}
	...
	private synchronized boolean acquireLock(ResourceAccess resourceAccess) {
		...
		waitingForResource = null;
		System.err
				.println("We didn't get the lock we wanted! How can that happen?");
		return false;
	}
	...
	private synchronized void abort() {
		owner.println("Aborting transaction " + transactionId + '.',
				transactionId);
		releaseLocks();
		owner.println("Transaction " + transactionId + " aborted.",
				transactionId);
	}
	...
	private synchronized void commit() {
		owner.println("Committing transaction " + transactionId + '.',
				transactionId);
		releaseLocks();
		owner.println("Transaction " + transactionId + " committed.",
				transactionId);
	}

	/**
	 * Called by the server executing this transaction whenever a server in the
	 * distributed system has disconnected. If the transaction is accessing any
	 * resources on the server that disconnected, the transaction aborts.
	 * 
	 * @param serverId
	 *            The ID of the server that disconnected.
	 */
	synchronized void serverDisconnected(int serverId) {
		for (ResourceAccess lockedResource : lockedResources) {
			if (lockedResource.serverId == serverId) {
				owner.println(
						"Contact was lost with a server hosting a resource involved in transaction "
								+ transactionId + ". Transaction must abort.",
						transactionId);
	...
	/**
	 * Releases all locks held by this transaction, in the reverse order of the
	 ...
	 * 
	 * @param resource
	 *            The resource whose lock should be released.
	 */
	private void releaseLock(ResourceAccess resource) {
		try {
			if (resource.server.releaseLock(transactionId, resource.resourceId)) {
				owner.println("Unlocked resource " + resource.resourceId
						+ " at server " + resource.serverId, transactionId);
			} else {
				owner.println("Failed to unlock resource "
						+ resource.resourceId + " at server "
						+ resource.serverId, transactionId);
			}
		} catch (RemoteException re) {
			owner.println("Failed to unlock resource " + resource.resourceId
