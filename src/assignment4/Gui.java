...
/**
 * Graphical User Interface used by the servers in a distributed transactional
 * system.
 */
class Gui extends JFrame implements ActionListener {
	...
	/**
	 * The tabbed pane containing the output areas of the MAX_NOF_TABS most
	 * recent transactions
	 */
	private JTabbedPane transactionPane;
	/**
	 * Labels for displaying the number of aborted and committed transactions so
	 * far
	 */
	 ...
	 * @param resources
	 *            The resources hosted by this GUI's server.
	 * @param servers
	 *            The servers known by this GUI's server.
	 * @param server
	 *            The server that this GUI belongs to.
	 * @param pos
	 *            Integer from 0-3 signifying which corner of the screen to
	 *            place the GUI in.
	 */
	Gui(String title, List<Resource> resources, Map<Integer, Server> servers,
			ServerImpl server, int pos) {
		super(title);
		this.server = server;
	 ...
	 * 
	 * @return Whether or not the arrival of new transactions has been halted by
	 *         the user.
	 */
	...
	/**
	 * Updates the labels showing the number of aborted and committed
	 * transactions so far.
	 * 
	 * @param nofAborts
	 *            The number of aborted transactions so far.
	 * @param nofTransactions
	 *            The number of executed transactions so far.
	 */
	void updateTransactionCounters(int nofAborts, int nofTransactions) {
		nofAbortsLabel.setText("# of aborts so far: " + nofAborts);
		nofCommitsLabel.setText("# of commits so far: "
				+ (nofTransactions - nofAborts));
	}
	 ...
	 * 
	 * @param resources
	 *            The resources located on this GUI's server.
	 */
	 ...
	 * 
	 * @param servers
	 *            The servers known to this GUI's server.
	 */
	...
	private void placeComponents() {
		...
		resourceTable
				.setPreferredScrollableViewportSize(new Dimension(150, 160));
	...
	/**
	 * Called by the server when another server has started up, and asked this
	 * server to start as well.
	 */
	...
	/**
	 * Creates an output area for a new transaction and adds it to the tabbed
	 * pane. If this results in too many tabs, the oldest tab is removed.
	 * 
	 * @param transactionId
	...
	/**
	 * Outputs a text string relevant to a specific transaction to that
	 * transaction's output area, formated with a newline at the end.
	 * 
	 * @param s
	...
	/**
	 * Outputs a text string with general information, formatted with a newline
	 * at the end.
	 * 
	 * @param s
	 *            The information to display.
	 ...
	 * @param error
	 *            The error message displayed in the error dialog box.
	 */
	...
	private static class OutputArea extends JPanel {
		...
		/**
		 * Outputs a text string, with a newline added at the end, to this
		 * output area.
		 * 
		 * @param s
		 *            The information to display.
	...
	/**
	 * Data model specifying the contents of the resource table in the GUI.
	 */
	private static class ResourceTableModel extends AbstractTableModel {
		 ...
		 * 
		 * @param resources
		 *            The resources that the table should display.
		 */
		 ...
		 * 
		 * @param resources
		 *            The new list of resources to display.
		 */
