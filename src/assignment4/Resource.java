...
/**
 * A resource with an associated lock that can be held by only one transaction
 * at a time.
 */
class Resource {
	...
	/**
	 * Gives the lock of this resource to the requesting transaction. Blocks the
	 * caller until the lock could be acquired.
	 * 
	 * @param transactionId
	...
	synchronized boolean lock(int transactionId) {
		if (lockOwner == transactionId) {
			System.err.println("Error: Transaction " + transactionId
					+ " tried to lock a resource it already has locked!");
			return false;
		}
	 ...
	 * 
	 * @param transactionId
	 *            The ID of the transaction that wants to release lock. If this
	 *            transaction doesn't currently own the lock an error message is
	 *            displayed.
	...
	synchronized boolean unlock(int transactionId) {
		if (lockOwner == NOT_LOCKED || lockOwner != transactionId) {
			System.err.println("Error: Transaction " + transactionId
					+ " tried to unlock a resource without owning the lock!");
			return false;
		}
	 ...
	 * 
	 * @return An Integer containing the ID of the transaction currently holding
	 *         the lock, or NOT_LOCKED if the resource is unlocked.
	 */
	...
	/**
	 * Checks if this resource's lock is held by a transaction running on the
	 * specified server.
	 * 
	 * @param serverId
